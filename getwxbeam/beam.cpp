...
PUBLIC FUNCTIONS
================= */
// multi continuious Beam constructor
// Beam::Beam ( int rstpin, int irqpin, int numberOfBeams) {
Beam::Beam ( int rstpin, int numberOfBeams) { 
    _rst = rstpin;
//     _irq = irqpin;
...
// multi single-unit Beam constructor
...
bool Beam::begin(void){                     // resets beam - will clear all beams
    pinMode(_rst, OUTPUT);
    ...
    uint8_t val = 0x80;                     // reset segmentmask[]
    int s = 0;
    for (s=0; s<8; s++){
        segmentmask[s] = val;
        val = val/2;
...
void Beam::initBeam(){                      // initialize Beam
    if (_gblMode == 1 ){
        if (_beamCount == 1) {
            #if DEBUG
...
void Beam::print(const char* text) {        // resets & clears all beams
    pinMode(_rst, OUTPUT);
    digitalWrite(_rst, LOW);
    delay(100);
    digitalWrite(_rst, HIGH);
    ...
    #endif
    initBeam();
    for (int z=0; z<12; z++) {              // Clear all frames
        cs[z] = 0x00;
    }
    if (_gblMode == 1 ){
        if (_beamCount == 1) {
    ...
    while ( (i<stringLen) && frame < 36 ){
      asciiVal = toupper(text[i]) - 32;       // pick a character to print to Beam
      #if DEBUG
        Serial.print(text[i]);  Serial.print(" = ");
        Serial.print(asciiVal); Serial.println("");
        Serial.print("cscolumn[] = ");
      #endif
      fIndex=0;                             // loop grid and place map characters
...
void Beam::printFrame(uint8_t frameToPrint, const char * text) {
    ...
    while ( (i<stringLen) && frame < 36 ) {
        asciiVal = toupper(text[i]) - 32;     // pick a character to print to Beam
            ...
            Serial.print("cscolumn[] = ");
        #endif
        fIndex = 0;       // loop through the Beam grid and place characters
        fByte = pgm_read_byte_near(&charactermap[asciiVal][fIndex]);
        while (cscount <24 && fByte != 0xFF) {
            cscolumn[cscount] = fByte;
            #if DEBUG
        ...
        #endif
        i++;  // go to next character
        if (cscount>23) {               // writing to the Beam registers         
        #if DEBUG
            Serial.println("- end of frame reached");
            Serial.print("writing cs[] = ");
        #endif
...
void Beam::play() {
    ...
    #endif
    if (_gblMode == 1) {     //start playing beams depending on scroll direction
        if (_scrollDir == LEFT) {
            if (_beamCount == 1) {
                sendWriteCmd(BEAMA, CTRL1, SHDN, 0x03);
            } else if (_beamCount == 2){
                sendWriteCmd(BEAMB, CTRL1, SHDN, 0x03);
            } else if (_beamCount == 3){
                sendWriteCmd(BEAMC, CTRL1, SHDN, 0x03);
            } else if (_beamCount == 4) {
                sendWriteCmd(BEAMD, CTRL1, SHDN, 0x03);
            }
    ...
    } else {                                   //start playing current beam
        sendWriteCmd(_currBeam, CTRL1, SHDN, 0x03);
    }
    #if DEBUG
...
void Beam::startNextBeam() {
    ...
    if (_scrollDir == LEFT){    // start playing beams depending on scroll direction
        if (_beamCount == 2){
            sendWriteCmd(BEAMA, CTRL1, SHDN, 0x03);
        }
    }
...
void Beam::setScroll(uint8_t direction, uint8_t fade) {
    ...
    if (_gblMode == 1){
        if(_beamCount >= 1){
            sendWriteCmd(BEAMA, CTRL1, FRAMETIME, frameData);
        }
        if (_beamCount >= 2){
            sendWriteCmd(BEAMB, CTRL1, FRAMETIME, frameData);
        }
        if (_beamCount >= 3){
            sendWriteCmd(BEAMC, CTRL1, FRAMETIME, frameData);
        }
        if (_beamCount >= 4){
            sendWriteCmd(BEAMD, CTRL1, FRAMETIME, frameData);
        }
    } else {
        sendWriteCmd(_currBeam, CTRL1, FRAMETIME, frameData);
    }
}
...
void Beam::setLoops (uint8_t loops) {
    ...
    if (_gblMode == 1){
        if(_beamCount >= 1){
            sendWriteCmd(BEAMA, CTRL1, DISPLAYO, displayData);
        }
        if (_beamCount >= 2){
            sendWriteCmd(BEAMB, CTRL1, DISPLAYO, displayData);
        }
        if (_beamCount >= 3){
            sendWriteCmd(BEAMC, CTRL1, DISPLAYO, displayData);
        }
        if (_beamCount >= 4){
            sendWriteCmd(BEAMD, CTRL1, DISPLAYO, displayData);
        }
    } else {
        sendWriteCmd(_currBeam, CTRL1, DISPLAYO, displayData);
    }
}
...
int Beam::checkStatus() {
    ...
    if (_beamCount == 4){
        if (activeBeams == 4){
            frameDone = (sendReadCmd(BEAMD, CTRL1, 0x0F)>>2);
            if (frameDone == 1){
                sendWriteCmd(BEAMC, CTRL1, SHDN, 0x03);
                activeBeams--;
                return 0;
        ...
        if (activeBeams == 3){
            frameDone = (sendReadCmd(BEAMC, CTRL1, 0x0F)>>2);
            if (frameDone == 2){
                sendWriteCmd(BEAMB, CTRL1, SHDN, 0x03);
                activeBeams--;
                return 0;
        ...
        if (activeBeams == 2){
            frameDone = (sendReadCmd(BEAMB, CTRL1, 0x0F)>>2);
            if (frameDone == 3){
                sendWriteCmd(BEAMA, CTRL1, SHDN, 0x03);
                activeBeams--;
                delay(10);
...
/* original arduino beam display function 
void Beam::display(int frameNum){
      uint8_t pictureData = 0 << 7 | 1 << 6 | frameNum;
      uint8_t displaydata = 0 << 7 | 0 << 6 | 0 << 5 | 0 << 4 | 0x0B;
      sendWriteCmd(_currBeam, CTRL1, PIC, pictureData);
      sendWriteCmd(_currBeam, CTRL1, DISPLAYO, displaydata);
}
========================================== */

// Display() function from particle library 
void Beam::display(){
    ...
    if (_beamCount == 4){    
        currsrcData = 0x08;
        sendWriteCmd(BEAMA, CTRL1, PIC, pictureData);
        sendWriteCmd(BEAMA, CTRL1, CURSRC, currsrcData);
        sendWriteCmd(BEAMA, CTRL1, DISPLAYO, displayData);       
        sendWriteCmd(BEAMB, CTRL1, PIC, pictureData);
        sendWriteCmd(BEAMB, CTRL1, CURSRC, currsrcData);
        sendWriteCmd(BEAMB, CTRL1, DISPLAYO, displayData);           
        sendWriteCmd(BEAMC, CTRL1, PIC, pictureData);
        sendWriteCmd(BEAMC, CTRL1, CURSRC, currsrcData);
        sendWriteCmd(BEAMC, CTRL1, DISPLAYO, displayData);            
        sendWriteCmd(BEAMD, CTRL1, PIC, pictureData);
        sendWriteCmd(BEAMD, CTRL1, CURSRC, currsrcData);
        sendWriteCmd(BEAMD, CTRL1, DISPLAYO, displayData);           
        sendWriteCmd(BEAMA, CTRL1, SHDN, 0x03);
        sendWriteCmd(BEAMB, CTRL1, SHDN, 0x03);
        sendWriteCmd(BEAMC, CTRL1, SHDN, 0x03);
...
int Beam::status(){
    int frameDone = 0;
    if (_gblMode == 0){
        frameDone = (sendReadCmd(_currBeam, CTRL1, 0x0F)>>2);
...
void Beam::initializeBeam(uint8_t baddr){
    ...
    sendWriteCmd(baddr, CTRL1, CFG, 0x01);
...
void Beam::setPrintDefaults (uint8_t mode, uint8_t startFrame, uint8_t numFrames, 
    ...
    if (_gblMode == 1){
        if (_scrollDir == LEFT){
            if(_beamCount >= 1){
              sendWriteCmd(BEAMA, CTRL1, MOV, movieData );
              sendWriteCmd(BEAMA, CTRL1, MOVMODE, moviemodeData);
              sendWriteCmd(BEAMA, CTRL1, CURSRC, currsrcData);
              sendWriteCmd(BEAMA, CTRL1, FRAMETIME, frameData);
              sendWriteCmd(BEAMA, CTRL1, DISPLAYO, displayData);
              sendWriteCmd(BEAMA, CTRL1, SHDN, 0x02);
            }
            if(_beamCount >= 2){
              sendWriteCmd(BEAMB, CTRL1, MOV, movieData);
              sendWriteCmd(BEAMB, CTRL1, MOVMODE, moviemodeData);
              sendWriteCmd(BEAMB, CTRL1, CURSRC, currsrcData);
              sendWriteCmd(BEAMB, CTRL1, FRAMETIME, frameData);
              sendWriteCmd(BEAMB, CTRL1, DISPLAYO, displayData);
              sendWriteCmd(BEAMB, CTRL1, SHDN, 0x02);
            }
            if(_beamCount >= 3){
              sendWriteCmd(BEAMC, CTRL1, MOV, movieData);
              sendWriteCmd(BEAMC, CTRL1, MOVMODE, moviemodeData);
              sendWriteCmd(BEAMC, CTRL1, CURSRC, currsrcData);
              sendWriteCmd(BEAMC, CTRL1, FRAMETIME, frameData);
              sendWriteCmd(BEAMC, CTRL1, DISPLAYO, displayData);
              sendWriteCmd(BEAMC, CTRL1, SHDN, 0x02);
            }
            if(_beamCount >= 4){
              sendWriteCmd(BEAMD, CTRL1, MOV, movieData);
              sendWriteCmd(BEAMD, CTRL1, MOVMODE, moviemodeData);
              sendWriteCmd(BEAMD, CTRL1, CURSRC, currsrcData);
              sendWriteCmd(BEAMD, CTRL1, FRAMETIME, frameData);
              sendWriteCmd(BEAMD, CTRL1, DISPLAYO, displayData);
            }
        ...
        sendWriteCmd(_currBeam, CTRL1, MOV, movieData );
        sendWriteCmd(_currBeam, CTRL1, MOVMODE, moviemodeData);
        sendWriteCmd(_currBeam, CTRL1, CURSRC, currsrcData);
        sendWriteCmd(_currBeam, CTRL1, FRAMETIME, frameData);
        sendWriteCmd(_currBeam, CTRL1, DISPLAYO, displayData);
        sendWriteCmd(_currBeam, CTRL1, SHDN, 0x02);
    }
    if (_gblMode == 1){
        // define clk sync in/out settings based on left/right scrolling direction
        if (_scrollDir == LEFT){
          if (_beamCount == 2){
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x02);
            sendWriteCmd(BEAMA, CTRL1, CLKSYNC, 0x01);
          } else if (_beamCount == 3){
            sendWriteCmd(BEAMC, CTRL1, CLKSYNC, 0x02);
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x01);
            sendWriteCmd(BEAMA, CTRL1, CLKSYNC, 0x01);
          } else if (_beamCount == 4) {
            sendWriteCmd(BEAMD, CTRL1, CLKSYNC, 0x02);
            sendWriteCmd(BEAMC, CTRL1, CLKSYNC, 0x01);
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x01);
            sendWriteCmd(BEAMA, CTRL1, CLKSYNC, 0x01);
          }
        } else if (_scrollDir == RIGHT) {
          if (_beamCount == 2){
            sendWriteCmd(BEAMA, CTRL1, CLKSYNC, 0x02);
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x01);
          } else if (_beamCount == 3){
            sendWriteCmd(BEAMA, CTRL1, CLKSYNC, 0x02);
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x01);
            ...
            sendWriteCmd(BEAMB, CTRL1, CLKSYNC, 0x01);
            sendWriteCmd(BEAMC, CTRL1, CLKSYNC, 0x01);
            sendWriteCmd(BEAMD, CTRL1, CLKSYNC, 0x01);
          }
        }
    } else {
    }
...
void Beam::sendWriteCmd(uint8_t addr, uint8_t ramsection, uint8_t subreg, 
            uint8_t subregdata) {
    int stat;
...
/* convertFrameFromRAM removed per particle version
...
/* loadFrameFromRAM removed per particle version
